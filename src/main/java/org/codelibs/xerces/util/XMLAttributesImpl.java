...
public class XMLAttributesImpl implements XMLAttributes {
    ...
    public int addAttribute(QName name, String type, String value) {
        ...
        if (fLength < SIZE_LIMIT) {
            index = name.uri != null && name.uri.length() != 0 ? getIndexFast(name.uri, name.localpart) : getIndexFast(name.rawname);
             ...
             * then adds more.
             */
            if (!fIsTableViewConsistent || fLength == SIZE_LIMIT || (fLength > SIZE_LIMIT && fLength > fTableViewBuckets)) {
    ...
    public int getIndex(String uri, String localPart) {
        for (int i = 0; i < fLength; i++) {
            ...
            if (attribute.name.localpart != null && attribute.name.localpart.equals(localPart)
                    && ((uri == attribute.name.uri) || (uri != null && attribute.name.uri != null && attribute.name.uri.equals(uri)))) {
                return i;
            }
        }
        return -1;
