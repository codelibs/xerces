...
class XSDGroupTraverser extends XSDAbstractParticleTraverser {
    ...
    XSParticleDecl traverseLocal(Element elmNode, XSDocumentInfo schemaDoc, SchemaGrammar grammar) {
        ...
        if (child != null) {
            reportSchemaError("s4s-elt-must-match.1", new Object[] { "group (local)", "(annotation?)", DOMUtil.getLocalName(elmNode) },
                    elmNode);
        }
        ...
        if (group != null && group.fModelGroup != null && !(minOccurs == 0 && maxOccurs == 0)) {
            ...
            if (group.fModelGroup.fCompositor == XSModelGroupImpl.MODELGROUP_ALL) {
                ...
                particle = checkOccurrences(particle, SchemaSymbols.ELT_GROUP, (Element) elmNode.getParentNode(), GROUP_REF_WITH_ALL,
    ...
    XSGroupDecl traverseGlobal(Element elmNode, XSDocumentInfo schemaDoc, SchemaGrammar grammar) {
        ...
        XSAnnotationImpl annotation = null;
        if (l_elmChild == null) {
            reportSchemaError("s4s-elt-must-match.2", new Object[] { "group (global)", "(annotation?, (all | choice | sequence))" },
                    elmNode);
        } else {
        ...
        if (group != null) {
            ...
            Object redefinedGrp = fSchemaHandler.getGrpOrAttrGrpRedefinedByRestriction(XSDHandler.GROUP_TYPE,
                    new QName(XMLSymbols.EMPTY_STRING, strNameAttr, strNameAttr, schemaDoc.fTargetNamespace), schemaDoc, elmNode);
            if (redefinedGrp != null) {
                // store in grammar
                grammar.addRedefinedGroupDecl(group, (XSGroupDecl) redefinedGrp, fSchemaHandler.element2Locator(elmNode));
            }
        }
