...
abstract class XSDAbstractTraverser {
    ...
    XSAnnotationImpl traverseSyntheticAnnotation(Element annotationParent, String initialContent, Object[] parentAttrs, boolean isGlobal,
    ...
    FacetInfo traverseFacets(Element content, XSTypeDefinition typeDef, XSSimpleType baseValidator, XSDocumentInfo schemaDoc) {
        ...
        while (content != null) {
            ...
            if (facet.equals(SchemaSymbols.ELT_ENUMERATION)) {
                ...
                // this facet element.
                if (enumVal == null) {
                    reportSchemaError("s4s-att-must-appear", new Object[] { SchemaSymbols.ELT_ENUMERATION, SchemaSymbols.ATT_VALUE },
                            content);
                ...
                if (child != null) {
                    reportSchemaError("s4s-elt-must-match.1", new Object[] { "enumeration", "(annotation?)", DOMUtil.getLocalName(child) },
                            child);
                }
                ...
                if (child != null) {
                    reportSchemaError("s4s-elt-must-match.1", new Object[] { "pattern", "(annotation?)", DOMUtil.getLocalName(child) },
                            child);
                }
            } else {
     ...
     */
    private void checkEnumerationAndMaxLengthInconsistency(XSSimpleType baseValidator, Vector enumData, Element contextNode,
            String typeName) {
     ...
     */
    private void checkEnumerationAndMinLengthInconsistency(XSSimpleType baseValidator, Vector enumData, Element contextNode,
            String typeName) {
        if (SchemaSymbols.URI_SCHEMAFORSCHEMA.equals(baseValidator.getNamespace())
                && SchemaSymbols.ATTVAL_HEXBINARY.equals(baseValidator.getName())) {
            for (int enumIdx = 0; enumIdx < enumData.size(); enumIdx++) {
            ...
            if (childName.equals(SchemaSymbols.ELT_ATTRIBUTE)) {
                ...
                if (tempAttrGrp.fAttributeWC != null) {
                    ...
                    // perform intersection of attribute wildcard
                        ...
                        attrGrp.fAttributeWC = attrGrp.fAttributeWC.performIntersectionWith(tempAttrGrp.fAttributeWC,
    ...
    void checkNotationType(String refName, XSTypeDefinition typeDecl, Element elem) {
        if (typeDecl.getTypeCategory() == XSTypeDefinition.SIMPLE_TYPE
            ...
            if ((((XSSimpleType) typeDecl).getDefinedFacets() & XSSimpleType.FACET_ENUMERATION) == 0) {
                reportSchemaError("enumeration-required-notation", new Object[] { typeDecl.getName(), refName, DOMUtil.getLocalName(elem) },
                        elem);
            }
        }
    }
    ...
    protected XSParticleDecl checkOccurrences(XSParticleDecl particle, String particleName, Element parent, int allContextFlags,
            long defaultVals) {
        ...
        if (processingAllEl) {
            if (max != 1) {
                reportSchemaError("cos-all-limited.2",
                        new Object[] { (max == SchemaSymbols.OCCURRENCE_UNBOUNDED) ? SchemaSymbols.ATTVAL_UNBOUNDED : Integer.toString(max),
