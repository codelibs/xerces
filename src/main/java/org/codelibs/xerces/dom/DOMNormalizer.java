 ...
 * @version $Id: DOMNormalizer.java 1710695 2015-10-26 20:48:54Z mrglavas $
 */
public class DOMNormalizer implements XMLDocumentHandler {
    ...
    protected void normalizeDocument(CoreDocumentImpl document, DOMConfigurationImpl config) {
        ...
        if (fValidationHandler != null) {
            ...
            fValidationHandler.startDocument(new SimpleLocator(fDocument.fDocumentURI, fDocument.fDocumentURI, -1, -1), fDocument.encoding,
                    fNamespaceContext, null);
            fValidationHandler.xmlDecl(fDocument.getXmlVersion(), fDocument.getXmlEncoding(), fDocument.getXmlStandalone() ? "yes" : "no",
                    null);
        }
        try {
    ...
    protected Node normalizeNode(Node node) {
        ...
        case Node.TEXT_NODE: {
            ...
            if (next != null && next.getNodeType() == Node.TEXT_NODE) {
                ...
                if (nextType == -1 || !(((fConfiguration.features & DOMConfigurationImpl.ENTITIES) == 0 && nextType == Node.ENTITY_NODE)
                        || ((fConfiguration.features & DOMConfigurationImpl.COMMENTS) == 0 && nextType == Node.COMMENT_NODE)
                        || ((fConfiguration.features & DOMConfigurationImpl.CDATA) == 0) && nextType == Node.CDATA_SECTION_NODE)) {
                    if (fDocument.errorChecking && ((fConfiguration.features & DOMConfigurationImpl.WELLFORMED) != 0)) {
                        isXMLCharWF(fErrorHandler, fError, fLocator, node.getNodeValue(), fDocument.isXML11Version());
                    }
                    if (fValidationHandler != null) {
    ...
    protected final void namespaceFixUp(ElementImpl element, AttributeMap attributes) {
        ...
        if (uri != null) { // Element has a namespace
            ...
            if (element.getLocalName() == null) {
                ...
                if (fNamespaceValidation) {
                    String msg = DOMMessageFormatter.formatMessage(DOMMessageFormatter.DOM_DOMAIN, "NullLocalElementName",
            ...
            for (int i = 0; i < fAttributeList.size(); i++) {
                ...
                if (fDocument.errorChecking && ((fConfiguration.features & DOMConfigurationImpl.WELLFORMED) != 0)) {
                    ...
                    if (fDocument.isXMLVersionChanged()) {
                        ...
                        if (!wellformed) {
                            String msg = DOMMessageFormatter.formatMessage(DOMMessageFormatter.DOM_DOMAIN,
                                    "wf-invalid-character-in-node-name", new Object[] { "Attr", attr.getNodeName() });
                            reportDOMError(fErrorHandler, fError, fLocator, msg, DOMError.SEVERITY_ERROR,
                                    "wf-invalid-character-in-node-name");
                        }
                    }
                ...
                if (uri != null) { // attribute has namespace !=null
                    ...
                    */
                    ...
                    if (attr.getLocalName() == null) {
                        ...
                        if (fNamespaceValidation) {
                            String msg = DOMMessageFormatter.formatMessage(DOMMessageFormatter.DOM_DOMAIN, "NullLocalAttrName",
        ...
        if (isXML11Version) {
            ...
            while (i < datalength) {
                ...
                if (XML11Char.isXML11Invalid(c)) {
                    ...
                    String msg = DOMMessageFormatter.formatMessage(DOMMessageFormatter.XML_DOMAIN, "InvalidCharInCDSect",
        ...
        if (isXML11Version) {
            ...
            while (i < datalength) {
                if (XML11Char.isXML11Invalid(dataarray[i++])) {
                    ...
                    String msg = DOMMessageFormatter.formatMessage(DOMMessageFormatter.DOM_DOMAIN, "InvalidXMLCharInDOM",
        ...
        if (isXML11Version) {
            ...
            while (i < datalength) {
                ...
                if (XML11Char.isXML11Invalid(c)) {
                    ...
                    String msg = DOMMessageFormatter.formatMessage(DOMMessageFormatter.XML_DOMAIN, "InvalidCharInComment",
                            new Object[] { Integer.toString(dataarray[i - 1], 16) });
                    reportDOMError(errorHandler, error, locator, msg, DOMError.SEVERITY_ERROR, "wf-invalid-character");
        ...
        if (a instanceof AttrImpl && ((AttrImpl) a).hasStringValue()) {
            ...
            for (int j = 0; j < children.getLength(); j++) {
                ...
                if (child.getNodeType() == Node.ENTITY_REFERENCE_NODE) {
                    ...
                    if (ent == null) {
                        String msg = DOMMessageFormatter.formatMessage(DOMMessageFormatter.DOM_DOMAIN, "UndeclaredEntRefInAttrValue",
    ...
    protected final class XMLAttributesProxy implements XMLAttributes {
        ...
        public int addAttribute(QName qname, String attrType, String attrValue) {
            ...
            if (index < 0) {
                ...
                // be added back
                AttrImpl attr = (AttrImpl) ((CoreDocumentImpl) fElement.getOwnerDocument()).createAttributeNS(qname.uri, qname.rawname,
                        qname.localpart);
