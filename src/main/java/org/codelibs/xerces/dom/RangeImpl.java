...
public class RangeImpl implements Range {
    ...
    public short compareBoundaryPoints(short how, Range sourceRange) throws DOMException {
        if (fDocument.errorChecking) {
            ...
            if ((fDocument != sourceRange.getStartContainer().getOwnerDocument() && fDocument != sourceRange.getStartContainer()
                    && sourceRange.getStartContainer() != null)
                    || (fDocument != sourceRange.getEndContainer().getOwnerDocument() && fDocument != sourceRange.getEndContainer()
                            && sourceRange.getStartContainer() != null)) {
                throw new DOMException(DOMException.WRONG_DOCUMENT_ERR,
                        DOMMessageFormatter.formatMessage(DOMMessageFormatter.DOM_DOMAIN, "WRONG_DOCUMENT_ERR", null));
            }
        ...
        for (Node pA = endPointA.getParentNode(), pB = endPointB.getParentNode(); pA != pB; pA = pA.getParentNode(), pB =
                pB.getParentNode()) {
            endPointA = pA;
            endPointB = pB;
        }
    ...
    public void insertNode(Node newNode) throws DOMException, RangeException {
        ...
        if (fStartContainer.getNodeType() == Node.TEXT_NODE) {
            ...
            ((TextImpl) cloneCurrent).setNodeValueInternal((cloneCurrent.getNodeValue()).substring(fStartOffset));
    ...
    public void surroundContents(Node newParent) throws DOMException, RangeException {
        ...
        if (fDocument.errorChecking) {
            ...
            if (type == Node.ATTRIBUTE_NODE || type == Node.ENTITY_NODE || type == Node.NOTATION_NODE || type == Node.DOCUMENT_TYPE_NODE
    ...
    public String toString() {
        ...
        if (fStartContainer.getNodeType() == Node.TEXT_NODE || fStartContainer.getNodeType() == Node.CDATA_SECTION_NODE) {
        ...
        if (fEndContainer.getNodeType() != Node.TEXT_NODE && fEndContainer.getNodeType() != Node.CDATA_SECTION_NODE) {
            int i = fEndOffset;
    ...
    private DocumentFragment traverseContents(int how) throws DOMException {
        ...
        for (Node sp = startNode.getParentNode(), ep = endNode.getParentNode(); sp != ep; sp = sp.getParentNode(), ep =
                ep.getParentNode()) {
            startNode = sp;
            endNode = ep;
        }
