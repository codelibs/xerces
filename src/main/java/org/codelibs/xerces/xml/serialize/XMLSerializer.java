                ...
                for (i = 0; i < attrs.getLength(); ++i) {
                    ...
                    if (name != null && name.length() == 0) {
                        ...
                        if ((attrURI != null && attrURI.length() != 0)
                                && (namespaceURI == null || namespaceURI.length() == 0 || !attrURI.equals(namespaceURI))) {
                    ...
                    if (value.equals(NamespaceContext.XMLNS_URI)) {
                        if (fDOMErrorHandler != null) {
                            ...
                            if (!continueProcess) {
                                ...
                                throw new RuntimeException(DOMMessageFormatter.formatMessage(DOMMessageFormatter.SERIALIZER_DOMAIN,
                    ...
                    if (fDOMErrorHandler != null) {
                        ...
                        if (!continueProcess) {
                            throw new RuntimeException(
                                    DOMMessageFormatter.formatMessage(DOMMessageFormatter.SERIALIZER_DOMAIN, "SerializationStopped", null));
                        }
                    }
        ...
        if (fNamespaces) {
            ...
            for (child = node.getFirstChild(); child != null; child = next) {
                ...
                if (fNSBinder.getURI(prefix) == null && prefix != null) {
                    fatalError("The replacement text of the entity node '" + node.getNodeName() + "' contains an element node '"
                            + child.getNodeName() + "' with an undeclared prefix '" + prefix + "'.");
                }
                ...
                if (child.getNodeType() == Node.ELEMENT_NODE) {
                    ...
                    for (int i = 0; i < attrs.getLength(); i++) {
                        ...
                        attrPrefix = (attrPrefix == null || attrPrefix.length() == 0) ? XMLSymbols.EMPTY_STRING
                                : fSymbolTable.addSymbol(attrPrefix);
                        if (fNSBinder.getURI(attrPrefix) == null && attrPrefix != null) {
                            fatalError("The replacement text of the entity node '" + node.getNodeName() + "' contains an element node '"
                                    + child.getNodeName() + "' with an attribute '" + attrs.item(i).getNodeName()
