...
public abstract class BaseTest {
    protected final static String ROOT_TYPE = Constants.XERCES_PROPERTY_PREFIX + Constants.ROOT_TYPE_DEFINITION_PROPERTY;

    protected final static String IGNORE_XSI_TYPE = Constants.XERCES_FEATURE_PREFIX + Constants.IGNORE_XSI_TYPE_FEATURE;

    protected final static String ID_IDREF_CHECKING = Constants.XERCES_FEATURE_PREFIX + Constants.ID_IDREF_CHECKING_FEATURE;

    protected final static String IDC_CHECKING = Constants.XERCES_FEATURE_PREFIX + Constants.IDC_CHECKING_FEATURE;

    protected final static String UNPARSED_ENTITY_CHECKING = Constants.XERCES_FEATURE_PREFIX + Constants.UNPARSED_ENTITY_CHECKING_FEATURE;
    ...
    protected final static String DYNAMIC_VALIDATION = Constants.XERCES_FEATURE_PREFIX + Constants.DYNAMIC_VALIDATION_FEATURE;

    protected final static String DOCUMENT_CLASS_NAME = Constants.XERCES_PROPERTY_PREFIX + Constants.DOCUMENT_CLASS_NAME_PROPERTY;
    ...
    protected void assertValidity(short expectedValidity, short actualValidity) {
        String expectedString = expectedValidity == ItemPSVI.VALIDITY_VALID ? "valid"
                : (expectedValidity == ItemPSVI.VALIDITY_INVALID ? "invalid" : "notKnown");
        String actualString = actualValidity == ItemPSVI.VALIDITY_VALID ? "valid"
                : (actualValidity == ItemPSVI.VALIDITY_INVALID ? "invalid" : "notKnown");
    ...
    protected void assertValidationAttempted(short expectedAttempted, short actualAttempted) {
        String expectedString = expectedAttempted == ItemPSVI.VALIDATION_FULL ? "full"
                : (expectedAttempted == ItemPSVI.VALIDATION_PARTIAL ? "partial" : "none");
        String actualString = actualAttempted == ItemPSVI.VALIDATION_FULL ? "full"
                : (actualAttempted == ItemPSVI.VALIDATION_PARTIAL ? "partial" : "none");
        String message = "{validity} was <" + actualString + "> but it should have been <" + expectedString + ">";
    ...
    protected void assertElementName(String expectedName, String actualName) {
        assertEquals(expectedName, actualName, "Local name of element declaration is wrong.");
    }
    ...
    protected void assertElementNamespace(String expectedName, String actualName) {
        assertEquals(expectedName, actualName, "Namespace of element declaration is wrong.");
    }
    ...
    protected void assertTypeName(String expectedName, String actualName) {
        assertEquals(expectedName, actualName, "Local name of type definition is wrong.");
    }
    ...
    protected void assertTypeNamespace(String expectedName, String actualName) {
        assertEquals(expectedName, actualName, "Namespace of type definition is wrong.");
    }
    ...
    protected void assertNoError(String error) {
        assertFalse(fErrorHandler.specialCaseFound(error), "Error <" + error + "> should not have occured (but it did)");
    }

    protected void assertAnyType(XSTypeDefinition type) {
        assertEquals(SchemaGrammar.fAnyType, type, "Type is supposed to be anyType");
    }
}
