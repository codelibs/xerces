...
public class XSParticleAnnotationsTest extends TestCase {
    ...
    private void synthAnnotationsTest(Boolean synth) {
        ...
        XSParticle element = (XSParticle) sequence.getParticles().item(0);
        XSObjectList annotations = element.getAnnotations();
        assertEquals((synth.booleanValue() == true) ? 1 : 0, annotations.getLength(), "TEST2_NO_ANNOTATIONS_" + synth);
    ...
    private void synthAllAnnotationsTest(Boolean synth) {
        String expected = trim("<annotation sn:attr1=\"SYNTH1\" ns2:attr2=\"SYNTH2\" " + "xmlns=\"http://www.w3.org/2001/XMLSchema\" "
                + "xmlns:sv=\"XSParticleTest\" xmlns:sn=\"SyntheticAnnotation\" " + "xmlns:ns2=\"TEST\" >"
        ...
        assertEquals((synth.booleanValue() == true) ? 1 : 0, annotations.getLength(), "TEST3_ANNOTATIONS_" + synth);
        if ((synth.booleanValue() == true))
            assertEquals(expected, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST3_ANNOTATIONS_" + synth);
    ...
    private void annotationsTest4(Boolean synth) {
        String expected = trim("<annotation sn:attr=\"SYNTH1\"  " + "id=\"ANNOT1\" xmlns=\"http://www.w3.org/2001/XMLSchema\" "
                + "xmlns:sv=\"XSParticleTest\" " + "xmlns:sn=\"SyntheticAnnotation\" >" + "</annotation>");
        String expected2 = trim("<annotation sn:attr=\"SYNTH1\"  " + "id=\"ANNOT2\" xmlns=\"http://www.w3.org/2001/XMLSchema\" "
                + "xmlns:sv=\"XSParticleTest\" " + "xmlns:sn=\"SyntheticAnnotation\" >" + "</annotation>");
        String expected3 = trim("<annotation " + "id=\"ANNOT3\" xmlns=\"http://www.w3.org/2001/XMLSchema\" "
                + "xmlns:sv=\"XSParticleTest\" " + "xmlns:sn=\"SyntheticAnnotation\" >" + "</annotation>");
        ...
        XSObjectList annotations = all.getAnnotations();
        assertEquals(expected, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST4_ANNOTATIONS_" + synth);

        XSParticle element = (XSParticle) all.getParticles().item(0);
        annotations = element.getAnnotations();
        assertEquals(expected2, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST4_ANNOTATIONS_2_" + synth);
        ...
        annotations = choice.getAnnotations();
        assertEquals(expected3, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST4_ANNOTATIONS_3_" + synth);
    }
    ...
    private void annotationsTest5(Boolean synth) {
        String expected = trim("<annotation id=\"ANNOT5\" " + "xmlns=\"http://www.w3.org/2001/XMLSchema\" " + "xmlns:sv=\"XSParticleTest\" "
                + "xmlns:sn=\"SyntheticAnnotation\" ></annotation>");
        String expected2 = trim("<annotation id=\"ANNOT4\" " + "xmlns=\"http://www.w3.org/2001/XMLSchema\" "
                + "xmlns:sv=\"XSParticleTest\" " + "xmlns:sn=\"SyntheticAnnotation\" ></annotation>");
        String expected3 = trim("<annotation sn:attr=\"SYNTH\"  " + "xmlns=\"http://www.w3.org/2001/XMLSchema\" "
        ...
        assertEquals(expected, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST5_ANNOTATIONS_1_" + synth);

        annotations = elemDecl2.getAnnotations();
        assertEquals(expected2, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST5_ANNOTATIONS_2_" + synth);
        ...
        if ((synth.booleanValue() == true)) {
            ...
            assertEquals(0, annotations.getLength(), "TEST5_ANNOTATIONS_3_" + synth);

            annotations = elemDecl3.getAnnotations();
            assertEquals(expected3, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST5_ANNOTATIONS_4_" + synth);
        }
    }
    ...
    private void annotationsTest6(Boolean synth) {
        String expected = trim("<annotation id=\"ANNOT7\" " + "xmlns=\"http://www.w3.org/2001/XMLSchema\" " + "xmlns:sv=\"XSParticleTest\" "
                + "xmlns:sn=\"SyntheticAnnotation\" ></annotation>");
        String expected2 = trim("<annotation id=\"ANNOT6\" " + "xmlns=\"http://www.w3.org/2001/XMLSchema\" "
                + "xmlns:sv=\"XSParticleTest\" " + "xmlns:sn=\"SyntheticAnnotation\" ></annotation>");
        String expected3 = trim("<annotation sn:attr=\"SYNTH\"  " + "xmlns=\"http://www.w3.org/2001/XMLSchema\" "
                + "xmlns:sv=\"XSParticleTest\" xmlns:sn=\"SyntheticAnnotation\" >"
                + "<documentation>SYNTHETIC_ANNOTATION</documentation></annotation>");
        String expected4 = trim("<annotation id=\"ANNOT8\" " + "xmlns=\"http://www.w3.org/2001/XMLSchema\" "
                + "xmlns:sv=\"XSParticleTest\" " + "xmlns:sn=\"SyntheticAnnotation\" ></annotation>");
        String expected5 = trim("<annotation id=\"ANNOT8\" " + "xmlns=\"http://www.w3.org/2001/XMLSchema\" "
                + "xmlns:sv=\"XSParticleTest\" " + "xmlns:sn=\"SyntheticAnnotation\" ></annotation>");
        String expected6 = trim("<annotation sn:attr=\"SYNTH\"  " + "xmlns=\"http://www.w3.org/2001/XMLSchema\" "
                + "xmlns:sv=\"XSParticleTest\" xmlns:sn=\"SyntheticAnnotation\" >"
                + "<documentation>SYNTHETIC_ANNOTATION</documentation></annotation>");
        String expected7 = trim("<annotation sn:attr=\"SYNTH\" id=\"ANNOT9\" " + "xmlns=\"http://www.w3.org/2001/XMLSchema\" "
                + "xmlns:sv=\"XSParticleTest\" " + "xmlns:sn=\"SyntheticAnnotation\" ></annotation>");
        ...
        XSElementDeclaration elemDecl2 = (XSElementDeclaration) elem2.getTerm();
        XSObjectList annotations = elem2.getAnnotations();
        assertEquals(expected, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST6_ANNOTATIONS_1_" + synth);

        annotations = elemDecl2.getAnnotations();
        assertEquals(expected2, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST6_ANNOTATIONS_2_" + synth);
        ...
        if (synth.booleanValue() == true) {
            ...
            annotations = elem3.getAnnotations();
            assertEquals(0, annotations.getLength(), "TEST6_ANNOTATIONS_3_" + synth);

            annotations = elemDecl3.getAnnotations();
            assertEquals(expected3, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST6_ANNOTATIONS_4_" + synth);
        }

        XSParticle elem3 = (XSParticle) sequencegrp.getParticles().item(2);
        XSElementDeclaration elemDecl3 = (XSElementDeclaration) elem3.getTerm();
        annotations = elem3.getAnnotations();
        assertEquals(expected4, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST6_ANNOTATIONS_1_" + synth);

        annotations = elemDecl3.getAnnotations();
        assertEquals(expected5, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST6_ANNOTATIONS_2_" + synth);
        ...
        if (synth.booleanValue() == true) {
            ...
            assertEquals(1, annotations.getLength(), "TEST6_ANNOTATIONS_3_" + synth);

            annotations = elemDecl4.getAnnotations();
            assertEquals(expected6, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST6_ANNOTATIONS_4_" + synth);
        }

        XSParticle any = (XSParticle) sequencegrp.getParticles().item(4);
        annotations = any.getAnnotations();
        assertEquals(expected7, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST6_ANNOTATIONS_1_" + synth);
    }
    ...
    private void annotationsTest7(Boolean synth) {
        String expected = trim("<annotation id=\"ANNOT10\" " + "xmlns=\"http://www.w3.org/2001/XMLSchema\" "
                + "xmlns:sv=\"XSParticleTest\" " + "xmlns:sn=\"SyntheticAnnotation\" >" + "</annotation>");
        String expected1 = trim("<annotation id=\"ANNOT11\" " + "xmlns=\"http://www.w3.org/2001/XMLSchema\" "
                + "xmlns:sv=\"XSParticleTest\" " + "xmlns:sn=\"SyntheticAnnotation\" >" + "</annotation>");
        String expected2 = trim("<annotation id=\"ANNOT12\" " + "xmlns=\"http://www.w3.org/2001/XMLSchema\" "
                + "xmlns:sv=\"XSParticleTest\" " + "xmlns:sn=\"SyntheticAnnotation\" >" + "</annotation>");
        String expected3 = trim("<annotation id=\"ANNOT14\" " + "xmlns=\"http://www.w3.org/2001/XMLSchema\" "
                + "xmlns:sv=\"XSParticleTest\" " + "xmlns:sn=\"SyntheticAnnotation\" >" + "</annotation>");
        String expected4 = trim("<annotation id=\"ANNOT15\" " + "xmlns=\"http://www.w3.org/2001/XMLSchema\" "
                + "xmlns:sv=\"XSParticleTest\" " + "xmlns:sn=\"SyntheticAnnotation\" >" + "</annotation>");
        String expected5 = trim("<annotation id=\"ANNOT13\" " + "xmlns=\"http://www.w3.org/2001/XMLSchema\" "
                + "xmlns:sv=\"XSParticleTest\" " + "xmlns:sn=\"SyntheticAnnotation\" >" + "</annotation>");
        ...
        assertEquals(expected, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST7_ANNOTATIONS_1_" + synth);

        XSModelGroup mg = (XSModelGroup) choice.getTerm();
        annotations = mg.getAnnotations();
        assertEquals(expected, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST7_ANNOTATIONS_2_" + synth);

        XSParticle seq = (XSParticle) mg.getParticles().item(0);
        annotations = seq.getAnnotations();
        assertEquals(expected1, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST7_ANNOTATIONS_3_" + synth);
        mg = (XSModelGroup) seq.getTerm();
        annotations = mg.getAnnotations();
        assertEquals(expected1, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST7_ANNOTATIONS_4_" + synth);

        XSParticle elem1 = (XSParticle) mg.getParticles().item(0);
        annotations = elem1.getAnnotations();
        assertEquals(expected2, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST7_ANNOTATIONS_5_" + synth);
        ...
        assertEquals(expected3, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST7_ANNOTATIONS_6_" + synth);
        mg = (XSModelGroup) all.getTerm();
        annotations = mg.getAnnotations();
        assertEquals(expected3, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST7_ANNOTATIONS_7_" + synth);

        XSParticle seq2 = (XSParticle) mg.getParticles().item(0);
        annotations = seq2.getAnnotations();
        assertEquals(expected4, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST7_ANNOTATIONS_8_" + synth);

        mg = (XSModelGroup) seq.getTerm();
        XSParticle any = (XSParticle) mg.getParticles().item(1);
        annotations = any.getAnnotations();
        assertEquals(expected5, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST7_ANNOTATIONS_9_" + synth);
