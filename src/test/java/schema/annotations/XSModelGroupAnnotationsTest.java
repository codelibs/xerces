...
public class XSModelGroupAnnotationsTest extends TestCase {
    ...
    private void group3AnnotationTest(Boolean synth) {
        String expected = trim("<annotation id=\"ANNOT1\" " + "xmlns=\"http://www.w3.org/2001/XMLSchema\" xmlns:sv=\"XSModelGroup\" "
        ...
        XSAnnotation annotation = group.getAnnotation();
        assertEquals(expected, trim(annotation.getAnnotationString()), "TEST3_NO_ANNOTATION_" + synth);
        XSObjectList annotations = group.getAnnotations();
        assertEquals(1, annotations.getLength(), "TEST3_NO_ANNOTATIONS_" + synth);
        assertEquals(expected, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST3_NO_ANNOTATIONS_" + synth);
    }
    ...
    private void group4AnnotationTest(Boolean synth) {
        String expected = trim("<annotation sn:att=\"SA1\"  id=\"ANNOT2\" " + "xmlns=\"http://www.w3.org/2001/XMLSchema\" "
                + "xmlns:sv=\"XSModelGroup\" xmlns:sn=\"SyntheticAnnotation\" >" + "</annotation>");
        ...
        XSAnnotation annotation = group.getAnnotation();
        assertEquals(expected, trim(annotation.getAnnotationString()), "TEST4_NO_ANNOTATION_" + synth);
        XSObjectList annotations = group.getAnnotations();
        assertEquals(1, annotations.getLength(), "TEST4_NO_ANNOTATIONS_" + synth);
        assertEquals(expected, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST4_NO_ANNOTATIONS_" + synth);
    }
    ...
    private void group5AnnotationTest(Boolean synth) {
        String expected1 = trim("<annotation sn:att=\"SA2\"  id=\"ANNOT3\" " + "xmlns=\"http://www.w3.org/2001/XMLSchema\" "
                + "xmlns:sv=\"XSModelGroup\" xmlns:sn=\"SyntheticAnnotation\" >" + "</annotation>");
        String expected2 = trim("<annotation sn:att=\"SA2\"  id=\"ANNOT4\" " + "xmlns=\"http://www.w3.org/2001/XMLSchema\" "
                + "xmlns:sv=\"XSModelGroup\" xmlns:sn=\"SyntheticAnnotation\" >" + "</annotation>");
        fConfig.setParameter("http://apache.org/xml/features/generate-synthetic-annotations", synth);
        ...
        XSAnnotation annotation = group.getAnnotation();
        assertEquals(expected1, trim(annotation.getAnnotationString()), "TEST5.1_NO_ANNOTATION_" + synth);
        XSObjectList annotations = group.getAnnotations();
        assertEquals(1, annotations.getLength(), "TEST5.1_NO_ANNOTATIONS_" + synth);
        assertEquals(expected1, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST5.1_NO_ANNOTATIONS_" + synth);

        group = (XSModelGroup) ((XSParticle) group.getParticles().item(0)).getTerm();
        annotation = group.getAnnotation();
        assertEquals(expected2, trim(annotation.getAnnotationString()), "TEST5.2_NO_ANNOTATION_" + synth);
        annotations = group.getAnnotations();
        assertEquals(1, annotations.getLength(), "TEST5.2_NO_ANNOTATIONS_" + synth);
        assertEquals(expected2, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST5.2_NO_ANNOTATIONS_" + synth);
    ...
    private void group6AnnotationTest(Boolean synth) {
        String expected1 = trim("<annotation sn:att=\"SA2\"  id=\"ANNOT5\" " + "xmlns=\"http://www.w3.org/2001/XMLSchema\" "
                + "xmlns:sv=\"XSModelGroup\" xmlns:sn=\"SyntheticAnnotation\" >" + "</annotation>");
        String expected2 = trim("<annotation sn:att=\"SA3\"  id=\"ANNOT6\" " + "xmlns=\"http://www.w3.org/2001/XMLSchema\" "
                + "xmlns:sv=\"XSModelGroup\" xmlns:sn=\"SyntheticAnnotation\" >" + "</annotation>");
        String expected3 = trim("<annotation id=\"ANNOT7\" " + "xmlns=\"http://www.w3.org/2001/XMLSchema\" "
                + "xmlns:sv=\"XSModelGroup\" xmlns:sn=\"SyntheticAnnotation\" >" + "</annotation>");
        ...
        XSAnnotation annotation = group.getAnnotation();
        assertEquals(expected1, trim(annotation.getAnnotationString()), "TEST6.1_NO_ANNOTATION_" + synth);
        XSObjectList annotations = group.getAnnotations();
        assertEquals(1, annotations.getLength(), "TEST6.1_NO_ANNOTATIONS_" + synth);
        assertEquals(expected1, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST6.1_NO_ANNOTATIONS_" + synth);

        group = (XSModelGroup) ((XSParticle) group.getParticles().item(0)).getTerm();
        annotation = group.getAnnotation();
        assertEquals(expected2, trim(annotation.getAnnotationString()), "TEST6.2_NO_ANNOTATION_" + synth);
        annotations = group.getAnnotations();
        assertEquals(1, annotations.getLength(), "TEST6.2_NO_ANNOTATIONS_" + synth);
        assertEquals(expected2, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST6.2_NO_ANNOTATIONS_" + synth);

        group = (XSModelGroup) ((XSParticle) group.getParticles().item(0)).getTerm();
        annotation = group.getAnnotation();
        assertEquals(expected3, trim(annotation.getAnnotationString()), "TEST6.3_NO_ANNOTATION_" + synth);
        annotations = group.getAnnotations();
        assertEquals(1, annotations.getLength(), "TEST6.3_NO_ANNOTATIONS_" + synth);
        assertEquals(expected3, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST6.3_NO_ANNOTATIONS_" + synth);
    ...
    private void group7AnnotationTest(Boolean synth) {
        String expected1 = trim("<annotation sn:att=\"SA1\"  id=\"ANNOT8\" " + "xmlns=\"http://www.w3.org/2001/XMLSchema\" "
                + "xmlns:sv=\"XSModelGroup\" xmlns:sn=\"SyntheticAnnotation\" >" + "</annotation>");
        String expected2 = trim("<annotation sn:att=\"SA1\"  " + "xmlns=\"http://www.w3.org/2001/XMLSchema\" "
        ...
        XSAnnotation annotation = group.getAnnotation();
        assertEquals(expected1, trim(annotation.getAnnotationString()), "TEST7.1_ANNOTATION_" + synth);
        XSObjectList annotations = group.getAnnotations();
        assertEquals(1, annotations.getLength(), "TEST7.1_ANNOTATIONS_" + synth);
        assertEquals(expected1, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST7.1_ANNOTATIONS_" + synth);

        if (synth == Boolean.TRUE) {
            group = (XSModelGroup) ((XSParticle) group.getParticles().item(0)).getTerm();
            annotation = group.getAnnotation();
            assertEquals(expected2, trim(annotation.getAnnotationString()), "TEST7.2_ANNOTATION_" + synth);
            annotations = group.getAnnotations();
            assertEquals(1, annotations.getLength(), "TEST7.2_ANNOTATIONS_" + synth);
            assertEquals(expected2, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST7.2_ANNOTATIONS_" + synth);
        }
    }
    ...
    private void group8AnnotationTest(Boolean synth) {
        ...
        XSAnnotation annotation = group.getAnnotation();
        assertEquals(expected, trim(annotation.getAnnotationString()), "TEST8_NO_ANNOTATION_" + synth);
        XSObjectList annotations = group.getAnnotations();
        assertEquals(1, annotations.getLength(), "TEST8_NO_ANNOTATIONS_" + synth);
        assertEquals(expected, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST8_NO_ANNOTATIONS_" + synth);
    }
    ...
    private void group9AnnotationTest(Boolean synth) {
        String expected = trim("<annotation sn:att=\"SA2\"  id=\"ANNOT12\" " + "xmlns=\"http://www.w3.org/2001/XMLSchema\" "
                + "xmlns:sv=\"XSModelGroup\" xmlns:sn=\"SyntheticAnnotation\" >" + "</annotation>");
        ...
        XSAnnotation annotation = group.getAnnotation();
        assertEquals(expected, trim(annotation.getAnnotationString()), "TEST9_NO_ANNOTATION_" + synth);
        XSObjectList annotations = group.getAnnotations();
        assertEquals(1, annotations.getLength(), "TEST9_NO_ANNOTATIONS_" + synth);
        assertEquals(expected, trim(((XSAnnotation) annotations.item(0)).getAnnotationString()), "TEST9_NO_ANNOTATIONS_" + synth);
    }
